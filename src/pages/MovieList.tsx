                    ...
                    {movies?.map((movie) => (
                        <li key={movie?.imdbID} className="bg-white rounded-b-[6px] text-black">
                            <Link className="h-full" to={`/movie/${movie?.imdbID}`}>
                                <div className="relative w-full xl:h-[300px] h-[420px]">
                                    {movie?.Poster !== "N/A" ? (
                                        <img className="w-full h-full" src={movie?.Poster} alt="img" />
                                    ) : (
                                    ...
                                    <div className="absolute top-0 z-[2] left-0" onClick={(e) => handleAddToWatchlist(e, movie)}>
                                        {watchlist?.some((el: any) => el?.imdbID === movie?.imdbID) ? (
                                            ...
                                            <BsBookmarkPlusFill className={`text-3xl p-0 opacity-[.8] ease-out duration-300 hover:opacity-[1] text-[#766a09]`} />
                                        )}
                                    </div>
                                </div>
                                <div className="p-5 ">
